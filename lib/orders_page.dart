...
class _OrdersNewPageState extends State<OrdersNewPage> with TickerProviderStateMixin {
  ...
  Widget _buildReadyToConfirmTab() {
    return Consumer<OrdersProvider>(
      builder: (context, ordersProvider, child) {
        if (ordersProvider.isReadyLoading) {
          return const Center(
        ...
        if (ordersProvider.readyOrders.isEmpty) {
          return const Center(
            child: Text(
              "No orders found",
                                                ...
                                                actions: [
                                                  ...
                                                  TextButton(
                                                    ...
                                                    onPressed: () async {
                                                      ...
                                                      final res = await ordersProvider.splitOrder(order.orderId, productSkus ?? [],
                                                          weightLimit: weightController.text.trim());
                                                      Navigator.pop(context);
                                                      Navigator.pop(context);
                                                      if (res['success'] == true) {
                                                        ordersProvider.showSnackBar(context, res['message'].toString(), Colors.green);
                                                      } else {
                                                        ordersProvider.showSnackBar(context, res['message'].toString(), Colors.red);
                                                      }
                                                    },
                                                  ),
                                                ],
  ...
  Widget _buildFailedOrdersTab() {
    return Consumer<OrdersProvider>(
      builder: (context, ordersProvider, child) {
        if (ordersProvider.isFailedLoading) {
          return const Center(
            child: LoadingAnimation(
              icon: Icons.shopping_cart,
              beginColor: Color.fromRGBO(189, 189, 189, 1),
        ...
        if (ordersProvider.failedOrders.isEmpty) {
          return const Center(
            child: Text(
              "No orders found",
              style: TextStyle(color: Colors.grey, fontSize: 16),
