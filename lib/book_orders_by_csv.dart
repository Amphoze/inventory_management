...
class BookOrdersByCsv extends StatefulWidget {
  const BookOrdersByCsv({super.key});

  @override
  State<BookOrdersByCsv> createState() => _BookOrdersByCsvState();
}

class _BookOrdersByCsvState extends State<BookOrdersByCsv> {
  ...
  void _showSnackbar(String message, Color color) {
    if (context.mounted) {
      Utils.showSnackBar(context, message, color: color, seconds: 3);
    }
  }
  ...
  Future<void> _bookOrders() async {
    ...
    try {
      ...
      Logger().e('Book Csv Body: $responseBody, Status Code: ${response.statusCode}');
      ...
      log('Error during boooking orders: $e', error: e, stackTrace: StackTrace.current);
      Utils.showSnackBar(context, 'Error while booking orders', details: e.toString(), isError: true);
            ...
            Row(
              children: [
                ...
                if (_rowCount > 0)
                  Expanded(
                    child: ElevatedButton(
                      onPressed: _isCreateEnabled && !_isCreating && !_isPickingFile && !_isProcessingFile ? _bookOrders : null,
                      child: const Text('Book Orders'),
                    ),
                  ),
              ],
            ),
